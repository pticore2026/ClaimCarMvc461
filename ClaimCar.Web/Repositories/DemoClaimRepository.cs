...
namespace ClaimCar.Web.Repositories
{
    public class DemoClaimRepository : IClaimRepository
    {
        ...
        private static readonly List<Claim> Claims = new List<Claim>
        {
            ...
            new Claim { Id=3, ManagementUnitCode="001", ManagementUnitName="thành phố Hồ Chí Minh", ManagementAreaCode="HCM", ManagementAreaName="Khu vực TP Hồ Chí Minh", LicensePlate="51H-888.68", EntryDate=new DateTime(2026,9,3), CallEntryDate=new DateTime(2026,9,3), PolicyNumber="HD-XCG-2026-003", Status="Mới tiếp nhận", AccidentDate=new DateTime(2026,9,1), NotificationDate=new DateTime(2026,9,3), ClaimNumber="HS-DEMO-003", SurveyorCode="GDV003", InsuredValue=850000000m },
            new Claim { Id=4, ManagementUnitCode="001", ManagementUnitName="thành phố Hồ Chí Minh", ManagementAreaCode="HCM", ManagementAreaName="Khu vực TP Hồ Chí Minh", LicensePlate="30K-567.89", EntryDate=new DateTime(2026,9,3), CallEntryDate=new DateTime(2026,9,3), PolicyNumber="HD-XCG-2026-004", Status="Đang giám định", AccidentDate=new DateTime(2026,8,30), NotificationDate=new DateTime(2026,9,3), ClaimNumber="HS-DEMO-004", SurveyorCode="GDV004", InsuredValue=1200000000m },
            new Claim { Id=5, ManagementUnitCode="001", ManagementUnitName="thành phố Hồ Chí Minh", ManagementAreaCode="HCM", ManagementAreaName="Khu vực TP Hồ Chí Minh", LicensePlate="43A-246.80", EntryDate=new DateTime(2026,9,3), CallEntryDate=new DateTime(2026,9,3), PolicyNumber="HD-XCG-2026-005", Status="Đã bảo lãnh", AccidentDate=new DateTime(2026,8,28), NotificationDate=new DateTime(2026,9,3), ClaimNumber="HS-DEMO-005", SurveyorCode="GDV005", InsuredValue=675000000m },
            new Claim { Id=6, ManagementUnitCode="001", ManagementUnitName="thành phố Hồ Chí Minh", ManagementAreaCode="HCM", ManagementAreaName="Khu vực TP Hồ Chí Minh", LicensePlate="92C-135.79", EntryDate=new DateTime(2026,9,3), CallEntryDate=new DateTime(2026,9,3), PolicyNumber="HD-XCG-2026-006", Status="Đã trình duyệt", AccidentDate=new DateTime(2026,8,25), NotificationDate=new DateTime(2026,9,3), ClaimNumber="HS-DEMO-006", SurveyorCode="GDV006", InsuredValue=540000000m }
        };
        ...
        private static QuoteViewModel BuildQuote(int id){ return new QuoteViewModel{ClaimId=id,ApprovalType="Duyệt giá",CompensationMethod="Bộ phận",ActualValue=991000000m,Items=new List<QuoteItem>{new QuoteItem{Id=1,PartName="Vỏ đèn hậu (hoặc miếng ngoài) bên phải",Quantity=0,Proposal="Sửa chữa",PartPrice=0,PaintCost=0,LaborCost=0},new QuoteItem{Id=2,PartName="Ốp cản sau phải",Quantity=1,Proposal="Sửa chữa",PartType="Chính hãng",PartPrice=160000,PaintCost=630000,LaborCost=0},new QuoteItem{Id=3,PartName="Tai xe phải",Quantity=1,Proposal="Sửa chữa",PartType="Chính hãng",PartPrice=50000,PaintCost=1060000,LaborCost=0}},RepairTotal=210000m,PaintTotal=1690000m,RepairDiscountPercent=5m,PaintDiscountPercent=5m,ParticipationValuePercent=100m,ParticipationFeePercent=100m,DeductibleCases=1,DeductibleAmount=462963m,CustomerPaymentTotal=462963m,ApprovedTotal=1342037m,Checker="HOANGLN2"}; }
        private static LossPaymentViewModel CloneLoss(LossPaymentViewModel x){return new LossPaymentViewModel{ClaimId=x.ClaimId,CauseCode=x.CauseCode,BehaviorCode=x.BehaviorCode,AreaCode=x.AreaCode,EventCode=x.EventCode,TbtnYcbReference=x.TbtnYcbReference,VehicleCertificateValue=x.VehicleCertificateValue,AccidentDescription=x.AccidentDescription,CauseDescription=x.CauseDescription,ConsequenceDescription=x.ConsequenceDescription,GarageCode=x.GarageCode,GarageName=x.GarageName,GaragePhone=x.GaragePhone,GarageEmail=x.GarageEmail,PayThroughGarage=x.PayThroughGarage,AssociationFund=x.AssociationFund,Coverages=(x.Coverages??new List<CoverageLine>()).Select(a=>new CoverageLine{Id=a.Id,CoverageCode=a.CoverageCode,Currency=a.Currency,InsuranceAmount=a.InsuranceAmount,LossPercent=a.LossPercent,LossAmount=a.LossAmount,Deductible=a.Deductible,CompensationAmount=a.CompensationAmount,TaxAmount=a.TaxAmount}).ToList(),OtherBeneficiaries=(x.OtherBeneficiaries??new List<BeneficiaryLine>()).Select(a=>new BeneficiaryLine{Id=a.Id,Code=a.Code,Name=a.Name,Currency=a.Currency,Amount=a.Amount}).ToList(),ThirdParties=(x.ThirdParties??new List<ThirdPartyLine>()).Select(a=>new ThirdPartyLine{Id=a.Id,Name=a.Name,Currency=a.Currency,Amount=a.Amount}).ToList()};}
        private static QuoteViewModel CloneQuote(QuoteViewModel x){return new QuoteViewModel{ClaimId=x.ClaimId,ApprovalType=x.ApprovalType,CompensationMethod=x.CompensationMethod,ActualValue=x.ActualValue,UncoveredDamageValue=x.UncoveredDamageValue,SubmitDate=x.SubmitDate,ReductionReason=x.ReductionReason,Items=(x.Items??new List<QuoteItem>()).Select(a=>new QuoteItem{Id=a.Id,PartCode=a.PartCode,PartName=a.PartName,Quantity=a.Quantity,Damage=a.Damage,Dimensions=a.Dimensions,Proposal=a.Proposal,PartType=a.PartType,PartPrice=a.PartPrice,PaintCost=a.PaintCost,LaborCost=a.LaborCost}).ToList(),ReplacementTotal=x.ReplacementTotal,SpecialReplacementTotal=x.SpecialReplacementTotal,RepairTotal=x.RepairTotal,PaintTotal=x.PaintTotal,LaborTotal=x.LaborTotal,TowingTotal=x.TowingTotal,NecessaryReasonableCost=x.NecessaryReasonableCost,RepairDiscountPercent=x.RepairDiscountPercent,PaintDiscountPercent=x.PaintDiscountPercent,LaborDiscountPercent=x.LaborDiscountPercent,ReplacementDiscountPercent=x.ReplacementDiscountPercent,ReplacementDepreciationPercent=x.ReplacementDepreciationPercent,SpecialDepreciationPercent=x.SpecialDepreciationPercent,ParticipationValuePercent=x.ParticipationValuePercent,ParticipationFeePercent=x.ParticipationFeePercent,DeductibleCases=x.DeductibleCases,DeductibleAmount=x.DeductibleAmount,SupplementalDeductibleAmount=x.SupplementalDeductibleAmount,CompensationReductionPercent=x.CompensationReductionPercent,RiskSharingPercent=x.RiskSharingPercent,CustomerPaymentTotal=x.CustomerPaymentTotal,ApprovedTotal=x.ApprovedTotal,Checker=x.Checker};}
        private static void CopyCalculationFields(QuoteViewModel source,QuoteViewModel target){target.SpecialReplacementDiscountPercent=source.SpecialReplacementDiscountPercent;target.TowingDiscountPercent=source.TowingDiscountPercent;target.CompensationReductionAmount=source.CompensationReductionAmount;}
    }
}
