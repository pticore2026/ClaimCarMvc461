...
namespace ClaimCar.Web.Controllers
{
    ...
    public class ClaimController : Controller
    {
        ...
        public ActionResult Edit(int id){var x=_service.Repository.Get(id);if(x==null)return HttpNotFound();return View(x);}
        [HttpPost,ValidateAntiForgeryToken]
        public ActionResult Edit(Claim x){if(ModelState.IsValid){var err=_service.ValidateClaim(x,x.Id==0?(int?)null:x.Id);if(err!=null)ModelState.AddModelError("",err);}if(!ModelState.IsValid)return View(x);try{var id=_service.SaveClaim(x);TempData["Success"]="Đã lưu thông tin chung.";return RedirectToAction("Edit",new{id=id});}catch(Exception ex){ModelState.AddModelError("",ex.Message);return View(x);}}
